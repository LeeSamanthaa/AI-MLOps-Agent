...
def initialize_session_state():
    ...
    for var in session_vars:
        if var not in st.session_state:
            if var in ["messages", "run_history", "parsed_actions"]:
                st.session_state[var] = []
            elif var in ["data_loaded", "ai_features_approved"]:
                st.session_state[var] = False
            elif var == "view":
                st.session_state[var] = "welcome"
            elif var == "config":
                st.session_state[var] = {"problem_type": "Classification"}
            else:
...
def handle_auto_config(df):
    ...
    with st.spinner("AI is analyzing your dataset and configuring the pipeline..."):
        ...
        response = generate_llm_response(
            prompt, st.session_state.config.get("groq_api_key"), is_json=True
        )
        try:
            ...
            if st.session_state.config.get("problem_type") != new_problem_type:
                ...
                st.session_state.config["selected_models"] = list(
                    get_models_and_search_spaces(new_problem_type).keys()
                )

            st.session_state.config["target"] = ai_config.get("target_column")
            st.session_state.config["numerical_features"] = ai_config.get(
                "numerical_features"
            )
            st.session_state.config["categorical_features"] = ai_config.get(
            ...
            st.error(
                f"AI configuration failed. The model's response could not be parsed. Details: {e}"
            )
...
def approve_features_callback():
    ...
    df_new = AIFeatureEngineer(st.session_state.feature_recommendations).transform(
        st.session_state.df
    )
    ...
    st.success(
        "Approved! The new features have been added and automatically selected in the dropdowns below."
    )
...
def display_welcome_page():
    ...
    st.markdown(
        "<div style='text-align: center;'>Your intelligent partner for solving <b>tabular classification and regression problems</b>. This agent doesn't just guide, it acts.</div>",
        unsafe_allow_html=True,
    )
    st.info(
        "This application automates the end-to-end machine learning lifecycle, from data analysis to model deployment. It leverages a Large Language Model to provide intelligent recommendations, translate complex metrics into business insights, and proactively guide you toward building better models."
    )
    ...
    with c2:
        ...
        dataset_choice = st.selectbox(
            "Choose a Sample Dataset", list(sample_datasets.keys())
        )
        if st.button(f"Load '{dataset_choice}' and Configure Pipeline"):
            st.session_state.df = sample_datasets[dataset_choice]
            st.session_state.config["problem_type"] = (
                "Classification" if "Classification" in dataset_choice else "Regression"
            )
            st.session_state.data_loaded = True
            st.session_state.view = "configuration"
            st.rerun()
...
def display_configuration_page():
    ...
    st.button(
        "Auto-Configure Pipeline with AI",
        on_click=handle_auto_config,
        args=(df,),
        use_container_width=True,
        help="Uses AI to analyze your dataset and recommend initial pipeline settings.",
    )
    if st.session_state.ai_config_rationale:
        st.info(
            f"**AI Configuration Rationale:** {st.session_state.ai_config_rationale}"
        )

    df_for_config = df.copy()
    if st.session_state.ai_features_approved:
        df_for_config = AIFeatureEngineer(
            st.session_state.feature_recommendations
        ).transform(df_for_config)
    ...
    with st.container(border=True):
        ...
        temp_target_fe = st.selectbox(
            "Select Target for Feature Engineering Analysis",
            df.columns,
            index=len(df.columns) - 1 if df.columns is not None else 0,
            key="ai_target_select_fe",
        )
        if st.button("Get AI Feature Recommendations"):
            if not st.session_state.config.get("groq_api_key"):
                ...
                with st.spinner("AI Agent is analyzing your data for feature ideas..."):
                    ...
                    recs = generate_llm_response(
                        prompt,
                        st.session_state.config.get("groq_api_key"),
                        is_json=True,
                    )
                    try:
                        st.session_state.feature_recommendations = json.loads(recs).get(
                            "recommendations"
                        )
                    except Exception:
        ...
        if st.session_state.feature_recommendations:
            for rec in st.session_state.feature_recommendations:
                st.info(
                    f"**Action:** {rec.get('technique', 'N/A').capitalize()} on **`{rec.get('column', 'N/A')}`**\n\n**Rationale:** {rec.get('rationale', 'Not provided.')}"
                )
    ...
    st.download_button(
        ...
        data=convert_df_to_csv(df_for_config),
    ...
    with col1:
        ...
        def on_problem_type_change():
            ...
            st.session_state.config["selected_models"] = list(
                get_models_and_search_spaces(problem_type).keys()
            )
        ...
        st.radio(
            ...
            index=["Classification", "Regression"].index(
                st.session_state.config.get("problem_type", "Classification")
            ),
        ...
        if MLFLOW_AVAILABLE:
            with st.container(border=True):
                ...
                st.session_state.config["mlflow_uri"] = st.text_input(
                    "MLflow Tracking URI",
                    value=st.session_state.config.get("mlflow_uri", "./mlruns"),
                )
                st.session_state.config["mlflow_experiment_name"] = st.text_input(
        ...
        default_target_val = st.session_state.config.get(
            "target", all_cols[-1] if all_cols else None
        )
        default_target_idx = (
            all_cols.index(default_target_val)
            if default_target_val in all_cols
            else len(all_cols) - 1
        )
        target_column = st.selectbox(
            "Select Target Variable", all_cols, index=default_target_idx
        )
        ...
        default_num = st.session_state.config.get(
            "numerical_features", detected["numerical"]
        )
        default_cat = st.session_state.config.get(
            "categorical_features", detected["categorical"]
        )

        num_features = st.multiselect(
            "Numerical Features", options=options_num, default=default_num
        )
        cat_features = st.multiselect(
            "Categorical Features", options=options_cat, default=default_cat
        )
    ...
    with col2:
        ...
        test_size = st.slider(
            ...
            0.1,
            0.5,
            st.session_state.config.get("test_size", 0.2),
            0.05,
        )
        cv_folds = st.number_input(
            "Cross-Validation Folds", 2, 10, st.session_state.config.get("cv_folds", 3)
        )
        handle_outliers = st.checkbox(
            "Handle Numerical Outliers (IQR Method)",
            value=st.session_state.config.get("handle_outliers", True),
        )
        imputation = st.selectbox(
            "Numerical Imputation", ["median", "mean", "most_frequent"], index=0
        )
        cat_imputation_strategy = st.selectbox(
            "Categorical Imputation", ["most_frequent", "constant"], index=0
        )
        scaler = st.selectbox("Scaler", ["StandardScaler", "MinMaxScaler"], index=0)

    with col3:
        st.subheader("Model Selection & Feature Selection")
        use_statistical_feature_selection = st.checkbox(
        ...
        default_models = [
            model
            for model in st.session_state.config.get("selected_models", all_models)
            if model in all_models
        ]
        selected_models = st.multiselect(
            "Select Models to Train", options=all_models, default=default_models
        )

        if problem_type == "Classification":
            primary_metric_display = st.selectbox(
                "Primary Optimization Metric", ["ROC AUC", "F1-Score"], index=0
            )
            ...
            primary_metric_display = st.selectbox(
                "Primary Optimization Metric", ["R2 Score", "Negative MSE"], index=0
            )
            metric_map = {"R2 Score": "r2", "Negative MSE": "neg_mean_squared_error"}
            imbalance = "None"

        bayes_iter = st.slider(
            "Hyperparameter Tuning Iterations",
            5,
            50,
            st.session_state.config.get("bayes_iterations", 15),
        )

    ensemble_model_name = (
        "Voting Classifier" if problem_type == "Classification" else "Voting Regressor"
    )
    ...
    if ensemble_model_name in selected_models:
        with st.container(border=True):
            ...
            ensemble_base_models = st.multiselect(
                "Select Base Models for Ensemble",
                options=base_model_options,
                default=st.session_state.config.get(
                    "ensemble_base_models",
                    [m for m in selected_models if m in base_model_options],
                ),
            )
    ...
    st.session_state.config.update(
        {
            ...
            "test_size": test_size,
            "cv_folds": cv_folds,
            "primary_metric": metric_map[primary_metric_display],
            "primary_metric_display": primary_metric_display,
            "imbalance_method": imbalance,
            "imputation_strategy": imputation,
            "cat_imputation_strategy": cat_imputation_strategy,
            "scaler": scaler,
            "handle_outliers": handle_outliers,
            "use_statistical_feature_selection": use_statistical_feature_selection,
            "k_features": k_features,
            "selected_models": selected_models,
            "bayes_iterations": bayes_iter,
    ...
    if st.button("Generate EDA Report", use_container_width=True):
        display_comprehensive_eda(
            df_for_config, num_features, cat_features, target_column
        )

    if st.button("Run Full MLOps Lifecycle", type="primary", use_container_width=True):
        if (
            not target_column
            or not (num_features or cat_features)
            or not selected_models
        ):
            st.error(
                "Please configure all required settings: Target Variable, Features, and at least one Model."
            )
        else:
            st.session_state.results_data = run_pipeline(
                st.session_state.df, st.session_state.config
            )
...
def display_results_page():
    ...
    results_df_sorted = results_df.sort_values(
        by=primary_metric_col, ascending=False
    ).reset_index(drop=True)
    best_model_row = results_df_sorted.iloc[0]
    best_model = best_model_row["model_object"]
    st.session_state.results_summary = results_df_sorted.drop(
    ...
    run_summary = {
        ...
        primary_metric_col: best_model_row[primary_metric_col],
    }
    ...
    with tabs[0]:
        ...
        if len(st.session_state.run_history) > 0:
            ...
            st.dataframe(
                history_df.style.highlight_max(
                    axis=0, color="#d4edda", subset=[primary_metric_col]
                )
            )
            if len(history_df) > 1:
                fig = px.line(
                    ...
                    title=f"Trend of {primary_metric_col} Across Runs",
                    markers=True,
                )
    ...
    with tabs[1]:
        ...
        summary = generate_llm_response(
            summary_prompt, st.session_state.config.get("groq_api_key")
        )
        ...
        parsed_actions_str = generate_llm_response(
            parser_prompt, st.session_state.config.get("groq_api_key"), is_json=True
        )
        try:
            st.session_state.parsed_actions = json.loads(parsed_actions_str).get(
                "actions", []
            )
        except (json.JSONDecodeError, AttributeError):
            st.session_state.parsed_actions = []

        if st.session_state.parsed_actions:
            with st.container(border=True):
                st.subheader("AI Action Panel")
                st.markdown(
                    "The AI Agent suggests the following improvements. Click to apply these settings and return to the configuration page."
                )
                for action in st.session_state.parsed_actions:
                    st.info(f"**Recommendation:** {action.get('action_text', 'N/A')}")
                st.button(
                    "Apply Recommendations & Return to Configuration",
                    on_click=apply_ai_recommendations,
                )

    with tabs[2]:
        st.subheader("Performance Metrics")
        st.dataframe(
            results_df_sorted.drop(columns="model_object").style.highlight_max(
                axis=0, color="#d4edda", subset=[primary_metric_col]
            )
        )
    ...
    with tabs[3]:
        ...
        if problem_type == "Classification" and len(np.unique(y_train)) == 2:
            st.info(
                "Adjust the classification threshold to see how it impacts business metrics."
            )
            ...
            if hasattr(best_model, "predict_proba"):
                ...
                st.metric(
                    "Potential Churners Missed (FN)",
                    f"{fn}",
                    delta=f"{-fn}",
                    delta_color="inverse",
                )
                st.metric(
                    "Loyal Customers Incorrectly Targeted (FP)",
                    f"{fp}",
                    delta=f"{-fp}",
                    delta_color="inverse",
                )
            ...
            st.info(
                "Business impact simulator is only available for binary classification problems."
            )
            ...
            with st.spinner("Calculating SHAP values..."):
                try:
                    if "Voting" in best_model_row["Model"]:
                        st.warning(
                            "SHAP plots for Voting ensembles are not directly supported."
                        )
                    else:
                        model_key = (
                            "classifier"
                        ...
                        explainer = shap.Explainer(
                            explainer_model, X_test_transformed_df
                        )
                        ...
                        shap.summary_plot(
                            shap_values, X_test_transformed_df, show=False
                        )
                        st.pyplot(fig)
    ...
    with tabs[6]:
        ...
        st.info(
            "Download the best model and a ready-to-use FastAPI script for deployment."
        )
        c1, c2 = st.columns(2)
        with c1:
        ...
        with c2:
            fastapi_script = generate_fastapi_script(
                st.session_state.config["numerical_features"],
                st.session_state.config["categorical_features"],
                problem_type,
            )
            st.download_button(
                label="Download FastAPI Script (.py)",
                data=fastapi_script,
...
def display_comprehensive_eda(df, numerical_features, categorical_features, target):
    ...
    with eda_tabs[0]:
        ...
        eda_summary_parts.append(
            f"Data Preview shows the first 10 rows. Columns are: {', '.join(df.columns)}."
        )
        ...
        if not missing_df.empty:
            ...
            st.plotly_chart(fig, use_container_width=True)
            eda_summary_parts.append(
                f"Missing Values found in: {', '.join(missing_df.index)}."
            )
        else:
            st.success("No missing values found.")
            eda_summary_parts.append("No missing values found.")
    ...
    with eda_tabs[1]:
        ...
        if numerical_features and len(numerical_features) > 1:
            corr_matrix = df[
                numerical_features + ([target] if target in df.columns else [])
    ...
    with eda_tabs[2]:
        ...
        if not categorical_features:
            ...
            for col in categorical_features:
                ...
                st.plotly_chart(fig, use_container_width=True)
            eda_summary_parts.append(
                f"Distributions for categorical features were plotted."
            )
...
def display_drift_monitoring_tab():
    ...
    if not ks_2samp:
        st.error(
            "Scipy library not found. Please install it (`pip install scipy`) for drift monitoring."
        )
        return

    st.info(
        "Upload a new dataset to compare its distribution against the training data."
    )
    new_data_file = st.file_uploader(
        "Upload New Data for Drift Analysis", type=["csv"], key="drift_uploader"
    )
        ...
        for col in st.session_state.config["numerical_features"]:
            if col in new_df.columns and col in baseline_stats["X_train_ref"].columns:
                ...
                if len(baseline_series) > 1 and len(new_series) > 1:
                    ...
                    if p_value < 0.05:
                        st.error(
                            f"Drift Detected! (Kolmogorov-Smirnov p-value: {p_value:.4f})"
                        )
                    else:
                        st.success(
                            f"No significant drift detected. (p-value: {p_value:.4f})"
                        )

                    fig_data = pd.DataFrame(
                        {
                            "value": pd.concat([baseline_series, new_series]),
                            "source": ["Baseline"] * len(baseline_series)
                            + ["New Data"] * len(new_series),
                        }
                    )
                    fig = px.histogram(
                        fig_data,
...
def display_sidebar_chat():
    ...
    with st.sidebar:
        ...
        st.write(
            "Ask questions about your data, model, or results. Answers are grounded in MLOps best practices and your specific context."
        )
        st.session_state.config["groq_api_key"] = st.text_input(
            "Groq API Key",
            type="password",
            key="sidebar_groq_api_key",
            value=st.session_state.config.get("groq_api_key", ""),
        )
        st.markdown("---")
        ...
        if prompt := st.chat_input("Ask about your data, model, or results..."):
            ...
            with st.chat_message("assistant"):
                ...
                with st.spinner("Agent is thinking..."):
                    ...
                    results_context = (
                        st.session_state.ai_summary or "No model has been run yet."
                    )
                    ...
                    response = generate_llm_response(
                        rag_prompt, st.session_state.config.get("groq_api_key")
                    )
