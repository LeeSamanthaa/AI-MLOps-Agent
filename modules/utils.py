...
def get_sample_datasets():
    ...
    churn_prob = (
        0.1
        + (df_c["Age"] / 100)
        - (df_c["IsActiveMember"] * 0.15)
        + (df_c["Geography"] == "Germany") * 0.1
        - (df_c["CreditScore"] / 10000)
    )
    ...
    df_r["Price"] = (
        150 * df_r["SquareFootage"]
        + 25000 * df_r["Bedrooms"]
        + 30000 * df_r["Bathrooms"]
        - 500 * (2025 - df_r["YearBuilt"])
...
class FeatureDetector:
    ...
    def detect_features(df, target=None):
        ...
        for col in df.columns:
            ...
            if df[col].nunique() / len(df) > 0.99 and pd.api.types.is_numeric_dtype(
                df[col]
            ):
...
def generate_fastapi_script(numerical_features, categorical_features, problem_type):
    ...
    """
    feature_schemas = ",\n    ".join(
        [f"{feat}: float" for feat in numerical_features]
        + [f"{feat}: str" for feat in categorical_features]
    )
    df_creation_lines = ",\n        ".join(
        [
            f"'{feat}': [data.{feat}]"
